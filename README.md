...
- ✅ 日志自动裁剪，避免日志文件和 WebUI 实时输出长期占用过多空间 / 内存
...
为避免长期运行后日志过大，Web 服务会自动裁剪日志：

- `LOG_MAX_BYTES`：日志超过该大小时触发清理，默认 `2097152`（2 MiB）
- `LOG_KEEP_BYTES`：清理后保留最近日志大小，默认 `524288`（512 KiB）
- `LOG_API_TAIL_LINES`：WebUI 每次最多返回最近多少行，默认 `300`
- `LOG_MEMORY_MAX_BYTES`：单次运行实时输出在内存中最多保留多少字节，默认 `16000`
